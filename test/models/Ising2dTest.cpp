...
TEST_CASE("Ising2d generate()", "[ising]") {
  Ising2d ising;
  ising.generate();
  for (int i = 0; i < ising.nodesLength; i++) {
    for (int j = 0; j < ising.nodesLength; j++) {
      REQUIRE(ising.nodes[i][j].id != 0);
    }
  }
  for (int i = 0; i < ising.arcsLength; i++) {
    REQUIRE(ising.arcs[i].node1 != 0);
    REQUIRE(ising.arcs[i].node2 != 0);
  }
  ising.size = 1;
  ising.generate();
  REQUIRE(ising.arcsLength == 0);
  REQUIRE(ising.nodesLength == 1);
  ising.size = 2;
  ising.generate();
